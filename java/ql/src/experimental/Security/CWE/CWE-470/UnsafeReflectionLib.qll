...
class UnsafeReflectionSink extends DataFlow::ExprNode {
  UnsafeReflectionSink() {
    exists(MethodAccess ma |
      (
        ma.getMethod().hasQualifiedName("java.lang.reflect", "Constructor<>", "newInstance") and
        this.asExpr() = [ma.getQualifier(), ma.getAnArgument()]
        or
        ma instanceof MethodInvokeCall and
        this.asExpr() = [ma.getQualifier(), ma.getArgument(1)]
      )
    )
  }
}
...
predicate looksLikeInstantiateClassStep(DataFlow::Node fromNode, DataFlow::Node toNode) {
  ...
  |
    m.getReturnType() instanceof TypeClass and
    m.getName().toLowerCase().regexpMatch("instantiate|instance|create|make") and
    arg.getType() instanceof TypeString and
    arg = fromNode.asExpr() and
    ma = toNode.asExpr()
  )
